...
int main()
{
	...
	if((smid = shmget(key, size, IPC_CREAT | 0666)) == -1) //creates the mem space and id
	{
		perror("shmget");
		exit(1);
	}	
	...
	if((addr = shmat(smid, NULL, 0)) == (char *)-1) //pointer to space 
	{
		perror("shmat");
		exit(1);
	}	

	shm = addr; //data to be shared

	for(int i = 0; i < 5; i++) 
		*shm++ = 'A';
	shm = addr; //reset shm to point to the beginning to check first char
	
	while(*shm != 'B') //waiting for shared memory to change
		sleep(1);
